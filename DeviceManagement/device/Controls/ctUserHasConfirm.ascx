<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="ctUserHasConfirm.ascx.cs" Inherits="DeviceManagement.device.Controls.ctUserHasConfirm" %>
<style>
    #dialog-category {
        margin-bottom: -30px;
    }
...
<div>
    ...
    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12" style="margin-top: 5px;">
        ...
        <div class="jarviswidget jarviswidget-color-teal" id="hasconfirm" data-widget-colorbutton="false" data-widget-editbutton="false" data-widget-togglebutton="false" data-widget-deletebutton="false" data-widget-fullscreenbutton="false" data-widget-sortable="false" data-widget-attstyle="jarviswidget-color-teal">
            <header>
            ...
            <div>
                ...
                <div class="widget-body">
                    <div class="table-responsive smart-form" id="divListHasConfirmed" style="width: 100%; overflow: auto;">

                        <table id="tableListHasConfirmed" class="table table-striped table-bordered table-hover" style="margin-bottom: 0px; min-width: 1000px;">
                            <thead>
                                <tr>
...
<script>
    var ListHasConfirmed = {
        Keyword: $("#<%=inputSearch.ClientID%>"),
        DllType: $('#<%=ddlType.ClientID%>'),
        LoadData: function () {
            AJAXFunction.CallAjax("POST", "/device/confirmation/userinventorypage.aspx", "ListHasConfirmed", {
                type: ListHasConfirmed.DllType.attr("dataid"),
                keyword: ListHasConfirmed.Keyword.val(),
                inventoryid: UserPage.inventoryid
            }, function (response) {
                if (response.Status) {
                    ListHasConfirmed.ShowData(response.Data);
                    pageSetUp();
                }
            });
        },
        ...
        LoadListCategory: function () {
            ...
            a.click(function () {
                ...
                ListHasConfirmed.LoadData();
            });
            ul.append($('<li>').append(a));
            for (var i = 0; i < ListHasConfirmed.ListType.length; i++) {
                var a = $('<a href="javascript:void(0)">').text(ListHasConfirmed.ListType[i].Name);
                a.attr('dataid', ListHasConfirmed.ListType[i].ID);
                a.click(function () {
                    $('#<%=ddlType.ClientID%>').text($(this).text());
                    $('#<%=ddlType.ClientID%>').attr("dataid", $(this).attr("dataid"));
                    ListHasConfirmed.LoadData();
        ...
        CurrentStatus: function (currentstatus, reason) {
            ...
            var status = ListHasConfirmed.GetStatusConfirm(currentstatus);
        ...
        ShowData: function (data) {
            ...
            var table = $("#tableListHasConfirmed > tbody").empty();
            ...
            if (data.Data.length == 0) {
                ...
                for (var i = 0; i < data.Data.length; i++) {
                    ...
                    tr.append(td);
                    var td = ListHasConfirmed.CurrentStatus(data.Data[i].ConfirmStaus, data.Data[i].Reason);
                    tr.append(td);
                    table.append(tr);
                }
            }
    ...
    ListHasConfirmed.Keyword.keypress(function (event) {
    ...
    ListHasConfirmed.LoadListCategory();
</script>
