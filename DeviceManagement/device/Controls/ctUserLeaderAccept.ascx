<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="ctUserLeaderAccept.ascx.cs" Inherits="DeviceManagement.device.Controls.ctUserLeaderAccept" %>
<style>
    #dialog-category {
        margin-bottom: -30px;
    }
...
<div>
    <div class="smart-form" style="margin-bottom: 20px;">
        ...
        <header style="padding-bottom: 15px; margin-right: 0px;">
            List devices leader has accepted
        </header>
    </div>
    ...
    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12" style="margin-top: 5px;">
        ...
        <div class="jarviswidget jarviswidget-color-teal" id="leaderaccept" data-widget-colorbutton="false" data-widget-editbutton="false" data-widget-togglebutton="false" data-widget-deletebutton="false" data-widget-fullscreenbutton="false" data-widget-sortable="false" data-widget-attstyle="jarviswidget-color-teal">
            <header>
            ...
            <div>
                ...
                <div class="widget-body">
                    <div class="table-responsive smart-form" id="divLeaderAccept" style="width: 100%; overflow: auto;">

                        <table id="tableLeaderAccept" class="table table-striped table-bordered table-hover" style="margin-bottom: 0px; min-width: 1000px;">
                            <thead>
                                <tr>
...
<script>
    var LeaderAccept = {
        ...
        LoadData: function () {
            AJAXFunction.CallAjax("POST", "/device/confirmation/userinventorypage.aspx", "ListLeaderAccept", {
                type: LeaderAccept.DllType.attr("dataid"),
                keyword: LeaderAccept.Keyword.val(),
                inventoryid: UserPage.inventoryid
            }, function (response) {
                if (response.Status) {
                    LeaderAccept.ShowData(response.Data);
                    pageSetUp();
                }
            });
        },
        ...
        LoadListCategory: function () {
            ...
            a.click(function () {
                ...
                LeaderAccept.LoadData();
            });
            ul.append($('<li>').append(a));
            for (var i = 0; i < LeaderAccept.ListType.length; i++) {
                var a = $('<a href="javascript:void(0)">').text(LeaderAccept.ListType[i].Name);
                a.attr('dataid', LeaderAccept.ListType[i].ID);
                a.click(function () {
                    $('#<%=ddlType.ClientID%>').text($(this).text());
                    $('#<%=ddlType.ClientID%>').attr("dataid", $(this).attr("dataid"));
                    LeaderAccept.LoadData();
        ...
        CurrentStatus: function (currentstatus, reason) {
            ...
            var status = LeaderAccept.GetStatusConfirm(currentstatus);
        ...
        ShowData: function (data) {
            ...
            var table = $("#tableLeaderAccept > tbody").empty();
            ...
            if (data.Data.length == 0) {
                ...
                for (var i = 0; i < data.Data.length; i++) {
                    ...
                    tr.append(td);
                    var td = LeaderAccept.CurrentStatus(data.Data[i].ConfirmStaus, data.Data[i].Reason);
                    tr.append(td);
                    table.append(tr);
                }
            }
    ...
    LeaderAccept.Keyword.keypress(function (event) {
