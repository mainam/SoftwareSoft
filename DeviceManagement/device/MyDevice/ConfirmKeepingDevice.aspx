...
<script type="text/javascript">
    ...
    function LoadHeader() {
        ...
        tr.append('<th class="theadtable" style="width: 90px;">Transfer Date</th>');
    ...
    function LoadData(page) {
        ...
        AJAXFunction.CallAjax("POST", "device/MyDevice/ConfirmKeepingDevice.aspx", "LoadData",
            {
                type: type,
                status: status,
                keyword: keyword,
            ...
            function (obj) {
                if (obj.Status) {
                    ...
                    LoadTable(listdevices, (numberinpage * (page - 1)));
    ...
    function UpdateButton() {
        ...
        document.getElementById("btnConfirm").disabled = !(input.length > 0);
        document.getElementById("btnReject").disabled = !(input.length > 0);
    }

    function Reject() {
        input = $('#tablelistdevice > tbody').find('input[typecheckbox="itemdevice"][disabled!="disabled"]:checked');
        if (input.length == 0) {
            alertbox("Select device before confirm");
            return;
        }

        confirm("Confirmation", "Do you want to reject items selected?", "Reject", "Cancel", function () {
            var listid = [];
            for (var i = 0; i < input.length; i++)
                listid.push($(input[i]).attr("dataid"));
            AJAXFunction.CallAjax("POST", "/device/mydevice/confirmkeepingdevice.aspx", "Submit", {
                listid: listid,
                reject: true
            }, function (response) {
                if (response.Status) {
                    alertSmallBox("Reject successful!", "1 second ago!!", "success");
    ...
    function Confirm() {
        ...
        confirm("Confirmation", "Do you want to confirm you are keeping these items selected?", "Confirm", "Cancel", function () {
            var listid = [];
            for (var i = 0; i < input.length; i++)
                listid.push($(input[i]).attr("dataid"));
            AJAXFunction.CallAjax("POST", "/device/mydevice/confirmkeepingdevice.aspx", "Submit", {
...
<section id="widget-grid" class="">
    ...
    <div class="row" style="margin-bottom: 5px;">
        <div class="alert alert-info alert-block" style="margin-left: 10px;">
            <h4 class="alert-heading">Confirm keeping device</h4>
        </div>
    ...
    <div class="row">
                        ...
                        <div class="table-responsive smart-form">
                            <div style="min-height: 500px; overflow: auto;">
                                <div style="overflow: auto;">
                                    <table id="tablelistdevice" class="table table-striped table-bordered table-hover" style="min-width: 1700px;">
                                        <thead>
                                        </thead>
                                        <tbody>
                                        </tbody>
        ...
        <div class=" pull-right" style="margin-top: 10px;">
            <input type="button" id="btnReject" class="btn btn-danger" style="float: right; width: 95px;" value="Reject" onclick="Reject();" />
            <input type="button" id="btnConfirm" class="btn btn-primary" style="margin-right: 10px; float: right; width: 95px;" value="Confirm" onclick="Confirm();" />
        </div>
