...
<div>
    ...
    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12" style="margin-top: 5px;">
        ...
        <div class="jarviswidget jarviswidget-color-teal" id="MyReject" data-widget-colorbutton="false" data-widget-editbutton="false" data-widget-togglebutton="false" data-widget-deletebutton="false" data-widget-fullscreenbutton="false" data-widget-sortable="false" data-widget-attstyle="jarviswidget-color-teal">
            <header>
            ...
            <div>
                ...
                <div class="widget-body">
                    <div class="table-responsive smart-form" id="divMyReject" style="width: 100%;">

                        <table id="tableMyReject" class="table table-striped table-bordered table-hover" style="margin-bottom: 0px; min-width: 1000px;">
                            <thead>
                                <tr>
...
<script>
    var MyReject = {
        ...
        LoadData: function () {
            AJAXFunction.CallAjax("POST", "/device/confirmation/leaderinventorypage.aspx", "ListHasRejected", {
                type: MyReject.DllType.attr("dataid"),
                keyword: MyReject.Keyword.val(),
                inventoryid: LeaderPage.inventoryid
            }, function (response) {
                if (response.Status) {
                    MyReject.ShowData(response.Data);
                    pageSetUp();
                }
            });
        },
        ...
        LoadListCategory: function () {
            ...
            for (var i = 0; i < MyReject.ListType.length; i++) {
                var a = $('<a href="javascript:void(0)">').text(MyReject.ListType[i].Name);
                a.attr('dataid', MyReject.ListType[i].ID);
                a.click(function () {
                    $('#<%=ddlType.ClientID%>').text($(this).text());
                    $('#<%=ddlType.ClientID%>').attr("dataid", $(this).attr("dataid"));
                    MyReject.LoadData();
        ...
        CurrentStatus: function (currentstatus, reason) {
            ...
            var status = MyReject.GetStatusConfirm(currentstatus);
        ...
        ShowData: function (data) {
            ...
            var table = $("#tableMyReject > tbody").empty();
            ...
            if (data.Data.length == 0) {
                ...
                for (var i = 0; i < data.Data.length; i++) {
                    ...
                    tr.append(td);
                    var td = MyReject.CurrentStatus(data.Data[i].ConfirmStaus, data.Data[i].Reason);
                    tr.append(td);
                    table.append(tr);
                }
            }
    ...
    MyReject.Keyword.keypress(function (event) {
