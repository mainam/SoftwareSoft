<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="ConfigTransferDeviceAllMember.aspx.cs" Inherits="DeviceManagement.Admin.Device.ConfigTransferDeviceAllMember" %>

<section id="widget-grid" class="">

    <div class="row" style="margin-left: 0px; margin-bottom: 5px;">
        <div class="alert alert-info alert-block">
            <h4 class="alert-heading">Set user allow transfer device for all member</h4>
        </div>
    </div>
    ...
    <div class="row" style="margin-bottom: 5px;">
        <article class="col-xs-12 col-sm-6 col-md-6 col-lg-6 sortable-grid ui-sortable">
            <div class="btn-group">
                <input class="btn btn-primary" type="button" value="Delete" style="margin-left: 10px; width: 95px" onclick="AllowTransferAllMember.Delete();" />
            </div>
        </article>
    ...
    <div class="row">
        ...
        <article class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
            ...
            <div class="jarviswidget  jarviswidget-color-teal" id="divListUserAllowTransferAllMember" data-widget-editbutton="false" data-widget-togglebutton="false" data-widget-deletebutton="false" data-widget-sortable="false" data-widget-attstyle="jarviswidget-color-teal">
                <header>
                    <span class="widget-icon"><i class="fa fa-table"></i></span>
                    <h2>List of user allow transfer device for all member</h2>
                    <div class="widget-toolbar" role="menu">
                        <!-- add: non-hidden - to disable auto hide -->
                        <div class="btn-group">
                            <input type="button" class="btn dropdown-toggle btn-xs btn-success btnwidth" value="Add" style="margin-right: 10px;" onclick="AllowTransferAllMember.Add();" />
                        </div>
                    </div>
                </header>
                <!-- widget div-->
...
<script>
    ...
    var AllowTransferAllMember = {
        ...
        Delete: function () {
            ...
            if (listcheckbox.length == 0) {
                ...
                confirm("Confirmation!", "Do you want to remove list users has selected", "OK", "Cancel", function () {
                    ...
                    AJAXFunction.CallAjax("POST", "/admin/device/ConfigTransferDeviceAllMember.aspx", "DeleteMultipleUser", { ListUser: listuser }, function (response) {
                        if (response.Status) {
                            alertSmallBox("Remove successful", "", "success");
                            AllowTransferAllMember.LoadData(AllowTransferAllMember.currentpage);
                        }
                        else {
        ...
        Add: function () {
            AJAXFunction.ShowModal("remoteModal2", "/admin/device/dialog/AddUserAllowTransferAllMember.aspx")
        },
            ...
            list.forEach(function (item) {
                ...
                label.click(function () {
                    ...
                    confirm("Confirmation!", "Do you want to remove this item", "OK", "Cancel", function () {
                        AJAXFunction.CallAjax("POST", "/admin/device/ConfigTransferDeviceAllMember.aspx", "Delete", { username: data }, function (response) {
                            if (response.Status) {
                                alertSmallBox("Remove successful", "", "success");
                                AllowTransferAllMember.LoadData(AllowTransferAllMember.currentpage);
                            }
        ...
        LoadData: function (page) {
            AllowTransferAllMember.currentpage = page;
            AJAXFunction.CallAjax("POST", "/admin/device/ConfigTransferDeviceAllMember.aspx", "LoadData", { keyword: AllowTransferAllMember.keyword, numberinpage: AllowTransferAllMember.numberinpage, currentpage: AllowTransferAllMember.currentpage }, function (response) {
                if (response.Status) {
                    AllowTransferAllMember.ShowData(response.Data);
                    var divtotalitem = $('#divtotalitem').empty();
                    divtotalitem.append('Total User: ' + response.TotalItem);
                    var totalpage = Common.GetTotalPage(AllowTransferAllMember.numberinpage, response.TotalItem);
                    AJAXFunction.CreatePaging($("#divpaging"), AllowTransferAllMember.currentpage, totalpage, AllowTransferAllMember.LoadData);
                }
            });
        }
    }
    AllowTransferAllMember.LoadData(1);

    $('#inputSearch').keydown(function (event) {
        var keycode = (event.keyCode ? event.keyCode : event.which);
        if (keycode == '13') {
            setTimeout(function () {
                AllowTransferAllMember.keyword = $('#inputSearch').val();
                AllowTransferAllMember.LoadData(1);
            });
        }
    });
