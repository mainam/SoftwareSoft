...
<div style="background: #fff; padding: 20px; margin: auto; clear: both; width: 900px; margin: auto; position: absolute; bottom: auto; left: 0; right: 0; vertical-align: middle;" id="divDialogAllowTransferAllMember">
    <div class="modal-header" style="height: 50px; background: #3276b1; text-align: center; margin-bottom: 20px; margin: -20px; color: white;">
        <span style="font-weight: bold; font-family: 'Open Sans',Arial,Helvetica,Sans-Serif; font-size: 20px; text-align: center;">SELECT USER
        </span>
    ...
    <div class="modal-body no-padding smart-form" style="margin: 20px auto 0px; background: white; padding: 30px; margin-top: 40px;">
        ...
        <div class="btn btn-sm btn-primary" style="float: right; margin: 2px; margin-top: 10px; width: 90px;" onclick="AddNewUser(); ">SAVE</div>
    </div>
</div>
<style>
    .select2 {
...
<script>
    $(document).ready(function () {
        $("#divDialogAllowTransferAllMember").delay(500).fadeIn(500).verticalAlign(400);
    });
    ...
    var AddNewUser = function () {
        ...
        AJAXFunction.CallAjax("POST", "/admin/device/dialog/AddUserAllowTransferAllMember.aspx", "AddNewMemberAllowTransferAllMember", { listuser: $("#<%=cbListMember.ClientID%>").val() }, function (response) {
            if (response.Status) {
