...
<section id="widget-grid" class="">
    ...
    <div class="row" style="margin-left: 0px; margin-bottom: 5px;">
        <div class="alert alert-info alert-block">
            <h4 class="alert-heading">Setting device manager</h4>
            Allow member manage device
        </div>
    </div>
    ...
    <div class="row" style="margin-bottom: 5px;">
        <article class="col-xs-12 col-sm-6 col-md-6 col-lg-6 sortable-grid ui-sortable">
            <div class="btn-group">
                <input class="btn btn-primary" type="button" value="Delete" style="margin-left: 10px; width: 95px" onclick="ConfigDeviceManager.Delete();" />
            </div>
        </article>
    ...
    <div class="row">
        ...
        <article class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
            ...
            <div class="jarviswidget  jarviswidget-color-teal" id="divconfigdevicemanager" data-widget-editbutton="false" data-widget-togglebutton="false" data-widget-deletebutton="false" data-widget-sortable="false" data-widget-attstyle="jarviswidget-color-teal">
                <header>
                    <span class="widget-icon"><i class="fa fa-table"></i></span>
                    <h2>List of members allow management device</h2>
                    <div class="widget-toolbar" role="menu">
                        <!-- add: non-hidden - to disable auto hide -->
                        <div class="btn-group">
                            <input type="button" class="btn dropdown-toggle btn-xs btn-success btnwidth" value="Add" style="margin-right: 10px;" onclick="ConfigDeviceManager.Add();" />
                        </div>
                    </div>
                </header>
                <!-- widget div-->
...
<script>
    ...
    var ConfigDeviceManager = {
        ...
        Delete: function () {
            ...
            if (listcheckbox.length == 0) {
                ...
                confirm("Confirmation!", "Do you want to remove list manager has selected", "OK", "Cancel", function () {
                    var listuser = [];
                    for (var i = 0; i < listcheckbox.length; i++) {
                        listuser.push($(listcheckbox[i]).attr("dataid"));
                        AJAXFunction.CallAjax("POST", "/admin/device/configdevicemanager.aspx", "DeleteMultipleManager", { ListManager: listuser }, function (response) {
                            if (response.Status) {
                                alertSmallBox("Remove successful", "", "success");
                                ConfigDeviceManager.LoadData(ConfigDeviceManager.currentpage);
                            }
                            else {
        ...
        Add: function () {
            AJAXFunction.ShowModal("remoteModal2", "/admin/device/dialog/adddevicemanager.aspx")
        },
            ...
            list.forEach(function (item) {
                ...
                label.click(function () {
                    ...
                    confirm("Confirmation!", "Do you want to remove this item", "OK", "Cancel", function () {
                        AJAXFunction.CallAjax("POST", "/admin/device/configdevicemanager.aspx", "Delete", { username: data }, function (response) {
                            if (response.Status) {
                                alertSmallBox("Remove successful", "", "success");
                                ConfigDeviceManager.LoadData(ConfigDeviceManager.currentpage);
                            }
        ...
        LoadData: function (page) {
            ConfigDeviceManager.currentpage = page;
            AJAXFunction.CallAjax("POST", "/admin/device/configdevicemanager.aspx", "LoadData", { keyword: ConfigDeviceManager.keyword, numberinpage: ConfigDeviceManager.numberinpage, currentpage: ConfigDeviceManager.currentpage }, function (response) {
                if (response.Status) {
                    ConfigDeviceManager.ShowData(response.Data);
                    var divtotalitem = $('#divtotalitem').empty();
                    divtotalitem.append('Total User: ' + response.TotalItem);
                    var totalpage = Common.GetTotalPage(ConfigDeviceManager.numberinpage, response.TotalItem);
                    AJAXFunction.CreatePaging($("#divpaging"), ConfigDeviceManager.currentpage, totalpage, ConfigDeviceManager.LoadData);
                }
            });
        }
    }
    ...
    $('#inputSearch').keydown(function (event) {
        ...
        if (keycode == '13') {
            setTimeout(function () {
                ConfigDeviceManager.keyword = $('#inputSearch').val();
                ConfigDeviceManager.LoadData(1);
            });
        }
    });
